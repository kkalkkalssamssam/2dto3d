...
public:
    ...
    void PubPointCloud(Gl::framedata_t frame_data, double theta, int idx);
...
void ChangeFinder::msgCallback(const std_msgs::Int16::ConstPtr & msg)
{
    if (isSaved()){
        if (msg->data == WAIT) {
            ...
            PubPointCloud(frame_data, theta, idx);
        }
    }
}
...
void ChangeFinder::PubPointCloud(Gl::framedata_t frame_data, double theta, int idx)
{
